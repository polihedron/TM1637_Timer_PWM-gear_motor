...
const int dir_pin = 8;                                                // PWM direction to gear motor at D8 pin       
...
unsigned long colon_ms, savemillis, timeLimit, lastAccelTime, lastDecelTime, lastPWMTime, timeRemaining;
...
void pwmCheck()  {
 ...
 int tempPWM = lastPWM;  
  ...
  if ( tempPWM <= dutyPWM ) {                                          // acceleration 
    if ( millis() - lastAccelTime > 100 ) {
      lastAccelTime = millis();
      tempPWM++;
    }
  }
  else if ( tempPWM > dutyPWM ) {
    if ( millis() - lastDecelTime > 10 ) {                             // dececeleration faster 10x than acc.
  ...
  if ( abs( lastPWM ) < 5 )  
    ...
    analogWrite( pwm_pin, map( abs( lastPWM ), 0, 100, 0, 255 ));       // remap 0-100% duty range to 0-255
}
